...
export const clioIntegration = new (class extends BaseTool {
  ...
  constructor() {
    ...
    if (!this.clioApiKey) {
      console.warn('CLIO_API_KEY not set. Clio integration will use mock data.');
    }
  }
  ...
  private getMockItemTracking(itemId: string) {
    const mockData = {
      id: itemId,
  ...
  private getMockMatterInfo(matterId: string) {
    const mockData = {
      id: matterId,
  ...
  private getMockWorkflowStatus(itemId: string) {
    const mockData = {
      item_id: itemId,
      current_stage: 'Attorney Review',
  ...
  private getMockClientInfo(clientId: string) {
    const mockData = {
      id: clientId,
      name: 'Smith, John',
  ...
  private getMockDocumentInfo(documentId: string) {
    const mockData = {
      id: documentId,
  ...
  private formatTrackingData(data: any) {
    return {
      id: data.id,
  ...
  private formatMatterInfo(data: any) {
    return {
      id: data.id,
      name: data.display_name || data.name,
      client: data.client?.name || 'Unknown Client',
  ...
  private formatWorkflowStatus(data: any) {
    return {
  ...
  private formatClientInfo(data: any) {
    return {
      id: data.id,
  ...
  private formatDocumentInfo(data: any) {
    return {
      id: data.id,
  ...
  private formatCalendarEvents(data: any) {
    return {
  ...
  private formatMatters(data: any) {
    return {
